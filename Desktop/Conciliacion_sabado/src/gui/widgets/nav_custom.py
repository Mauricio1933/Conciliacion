...
class NavBar(QWidget):
    ...
    conciliacion_clicked = pyqtSignal()
    ...
    ICON_NAV_MAP = {
        ...
        "Conciliación": "conciliacion_icon.svg",
        "Ayuda": "help_icon.svg",
        "Cerrar Sesión": "logout_icon.svg"
    }
    ...
    def __init__(self, parent=None):
        ...
        self.btn_conciliacion = NavButton(
            "Conciliación", 
            self.ICON_NAV_MAP["Conciliación"], 
            "navButtonConciliacion" 
        )
        
                               
        self.btn_registros = NavButton(
            "Registros", 
            self.ICON_NAV_MAP["Registros"], 
            "navButtonRegistros" 
        )
        
        self.btn_ayuda = NavButton(
            "Ayuda", 
            self.ICON_NAV_MAP["Ayuda"], 
            "navButtonData"
        )

        self.btn_logout = NavButton(
            "Cerrar Sesión",
            self.ICON_NAV_MAP["Cerrar Sesión"],
            "navButtonLogout"
        )

        self.btn_usuarios.clicked.connect(self.usuarios_clicked)
        self.btn_conciliacion.clicked.connect(self.conciliacion_clicked)
        self.btn_registros.clicked.connect(self.registros_clicked)
        self.btn_ayuda.clicked.connect(self.ayuda_clicked)
        self.btn_logout.clicked.connect(self.logout_clicked)
        
        nav_layout.addWidget(self.btn_usuarios)
        nav_layout.addWidget(self.btn_conciliacion)
        nav_layout.addWidget(self.btn_registros)
        nav_layout.addWidget(self.btn_ayuda)
    ...
    def hide_users_button(self):
        self.btn_usuarios.setVisible(False)
        
    def hide_conciliacion_button(self):
        self.btn_conciliacion.setVisible(False)