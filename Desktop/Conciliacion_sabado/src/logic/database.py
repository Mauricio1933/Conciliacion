...
def _normalize_roles():
    ...
    try:
        ...
        cursor.execute("UPDATE Usuarios SET rol = CASE WHEN LOWER(rol) = 'admin' THEN 'admin' ELSE 'asistente' END")
        conn.commit()
    except Exception:
        pass
    finally:
...
def add_user(first_name: str, last_name: str, username: str, email: str, password: str, role: str = "asistente") -> tuple[bool, str]:
    ...
    role_normalized = 'admin' if str(role).lower() == 'admin' else 'asistente'
    ...
    try:
        cursor.execute(
            "INSERT INTO Usuarios (nombre, apellido, nombre_usuario, correo, contraseña, rol, estado) VALUES (?, ?, ?, ?, ?, ?, ?)",
            (first_name, last_name, username, email, password_hash, role_normalized, "activo")
        )
        conn.commit()
